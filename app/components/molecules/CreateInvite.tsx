...
import { CreateInviteValues } from "@/app/utils/types/invite";
import { CreateInviteSchema } from "@/app/utils/yup";
import { useFormik } from "formik";
import { useRouter } from "next/navigation";

const initialValues: CreateInviteValues = {
  name: "",
  token: "",
};
...
const CreateInvite = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (!token) return;
      await createInvite({ name: values.name, token: token });
    },
  });
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="max-w-110">
      ...
      <input
        ...
        placeholder="invite someone with a name that hasn't been invited before."
